...
def test(model, device, test_loader, criterion, epoch, iter_meter):
    ...
    with torch.no_grad():
        for i, _data in enumerate(test_loader):
            ...
            decoded_preds, decoded_targets = preprocess.GreedyDecoder(output.transpose(0, 1), labels, label_lengths)
            for j in range(len(decoded_preds)):
                test_cer.append(preprocess.cer(decoded_targets[j], decoded_preds[j]))
...
def train_test_valid(hparams, batch_iterator):
    ...
    for epoch in range(1, epochs + 1):
        train(model, device, batch_iterator, criterion, optimizer, scheduler, epoch, iter_meter, batch_size)
